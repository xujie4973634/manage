...
public class HibernateDao {
    ...
    public Long save(Object model) {
        return (Long) this.getSession().save(model);
    }

    public <T> List<Long> batchSave(List<T> models) {
        List<Long> modelIds = new ArrayList<>(models.size());
        Session session = this.getSession();
        for (int i = 0; i < models.size(); i++) {
            Object model = models.get(i);
            Long modelId = (Long) session.save(model);
     ...
     * @return 删除数据的条数
     */
    public int delete(List<Long> modelIds, Class clazz) {
